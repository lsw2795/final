...
as
select c.*,e.name,e.image,d.form_name,d.security_level,d.deadline,s.state,confirm1,confirm2,confirm3,confirm1name,confirm2name,confirm3name from
...
join confirmlineview l
on c.confirm_line_no=l.confirm_line_no
order by c.confirm_document_no desc;
...
as
select c.*,f.form_name,e.name, a.agreement_state, d.name as deptname, d.manager, e2.name as managername, s.state from
