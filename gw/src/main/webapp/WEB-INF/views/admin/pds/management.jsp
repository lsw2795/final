...
<!-- 페이징 처리 관련 form -->
<form action="<c:url value='/admin/pds/management'/>" name="frmPage" method="post">
						...
						<div class="d-lg-flex justify-content-between">
								...
								<div class="row flex-between-center gy-2 px-x1">
									<div class="col-auto pe-0 admindefault">
										<select name="searchCondition" class="admindefault adminempborder adminempsel">
											<option value="fileName" <c:if test="${param.searchCondition=='fileName'}"> selected="selected" </c:if>>파일명</option>
							...
							<div class="d-flex align-items-center justify-content-between justify-content-lg-end px-x1 admindefault">
								<div class="d-flex align-items-center admindefault" id="table-contact-replace-element">
									<button id="btnDel" class="btn btn-falcon-default btn-sm admindefault" type="button">
										<span class="fas fa-minus" data-fa-transform="shrink-3"></span>
										<span class="d-none d-sm-inline-block d-xl-none d-xxl-inline-block ms-1">삭제</span>
									</button>
								</div>
							</div>
						</div>
							...
							<table class="table table-sm table-hover">
								...
								<tbody id="table-contact-body">
									<c:if test="${empty list}">
										<tr class="adminemptr">
											<td colspan="7" style="text-align: center">자료실에 파일이 존재하지 않습니다.</td>
										</tr>
									</c:if>
									<c:if test="${!empty list}">
												...
												<tr class="adminemptr">
													<td class="align-middle fs-0 py-3 align-middle">
														<div class="form-check mb-0">
															<input class="form-check-input" type="checkbox" data-bulk-select-row="data-bulk-select-row" name="pdsItems[${idx}].pdsNo" value=${map['PDS_NO']} />
														</div>
													</td>
